...
public class LoadData
{
    ...
    public static void plotLineChart(List list_1, List list_2, String chartname)
    {
        ...
        try
        {
            ...
            ChartUtilities.writeChartAsPNG(out, linechart,1000,1000);
    ...
    public static void kmeans(int k, int numIterations)
    {
        ...
        JavaRDD<ElectricPowerUtility> housePURDD = sparkSession
                .read().textFile(csv_file)
                .javaRDD().filter( str-> !(null==str))
                .filter(str-> !(str.length()==0))
                .filter(str-> !str.contains("Date"))
                .filter(str->!str.contains("?"))
                .filter(str->!(str.split(";").length<9))
                .map(str -> ElectricPowerUtility.parseRecord(str));
	   ...
	   // transform data into javaPairRdd
        ...
        JavaPairRDD<String, Tuple2<List<Float>,Integer>> houseweekRdd =  housePURDD.mapToPair(
                new PairFunction<ElectricPowerUtility, String , Tuple2<List<Float>,Integer>>()
                {   /**
                 *
                 */
                private static final long serialVersionUID = 1L;
                    ...
                    @Override
                    public Tuple2<String, Tuple2<List<Float>,Integer>> call(ElectricPowerUtility electricpu) throws Exception
                    {
                        Tuple2<String, Tuple2<List<Float>,Integer>> tuple = new Tuple2<String,
                                Tuple2<List<Float>,Integer>> (electricpu.getYearH()+"_"+electricpu.getWeekH(), new Tuple2<List<Float>,Integer>(electricpu.getElectricPVector(),1));
                        return tuple;
        ...
        JavaPairRDD<String, List<Float>> houseweekRddAvg =  housePURDD.mapToPair(
                ...
                private static final long serialVersionUID = 1L;
                    ...
                    public Tuple2<String, Tuple2<List<Float>,Integer>> call(ElectricPowerUtility electricpu) throws Exception
                    {
                        Tuple2<String, Tuple2<List<Float>,Integer>> tuple = new Tuple2<String, Tuple2<List<Float>,Integer>> (electricpu.getYearH()+"_"+electricpu.getWeekH(), new Tuple2<List<Float>,Integer>(electricpu.getElectricPVector(),1));
                        return tuple;
                    }
                }).reduceByKey(
